...
function AddImage() {
  ...
  const addImageData = () => {
    ...
    AxiosInstants.post("/users/AddImages", fileData, {
      params: formValue,
      headers: { "Content-Type": "multipart/form-data" },
    })
      .then((response) => {
        toastSuccess("added image successfully");
        navigate("/home");
      })
      .catch((err) => {
        console.log(err);
  ...
  //     "/users/AddImages",
  //     fileData,
  //     { params: formValue, userId: userId },
  //     { headers: { "content-Type": "multipart/form-data" } }
  //   )
  //     .then((response) => {
    ...
    <>
      ...
      <div className="container mt-5 d-grid p-3 text-center add-img-form">
        ...
        <label className="pt-5" htmlFor="images">
          select a image :
          <input type="file" name="images" id="images" onChange={onImage} />
          {selectedImage && (
            <img src={selectedImage} alt="" width={"100px"} height={"100px"} />
          )}
        </label>
        ...
        <div className="p-3 ">
          <button className=" btn btn-primary px-5 " onClick={addImageData}>
            submit
          </button>
        </div>
      </div>
